...
const armas = {
    ...
    EspadaLonga: { nome: "Espada Longa do Valor", forca: 2 },
...
function h1() {
    ...
    Thiago: Acabou, Cibely... O reino de Salinoria caiu, Paradisium está em ruínas, e Berizalém foi destruída ontem, com a chegada da Deusa das Trevas. O único império que ainda resiste é o grande império de Clarimontes.
...
function h2() {
    ...
    *Pedro Rocha se aproxima de Thiago e o esfaqueia com uma faca envenenada, fazendo-o desmaiar*
    Pedro Rocha: Você...
    Pedro Rocha: ELA sentenciou a sua morte.`);
    skipFala()
}
...
function h3() {
    ...
    Cibely: Sim, e eu ainda adquiri uma nova habilidade quando matei ele, o ${personagens.habilidades[1].nome}.
    Thiago: Amassou, Cibs.
    Cibely: 😝
    ...
    Cibely: Ele disse algo sobre como eu jamais iria conseguir vencer ELA e que a esse ponto nem a ${naturezas.arma} seria capaz de destruir sua rede neural e que a esse ponto ela já teria roubado do reino de Taioberen.
    Thiago: Então a ${naturezas.arma} está lá? Eu ouvi rumores, mas não acreditei que era real...
    Cibely: O que seria essa tal de ${naturezas.arma}?
    Thiago: É uma arma mortal, capaz de fazer até o guerreiro mais fraco 4 vezes mais forte.
    ...
    // Bloco 3.3
    mecanismoFala("Confronto no Castelo", `*No castelo*
    ...
    Cibely: Teoricamente, eu matei o líder deles só no chute. Kkkkkkkkk, muito otário.
    Tharcísio: Quanta bobagem... Vocês esperam que eu acredite que uma garota de 17 anos seria capaz de matar os ditos soldados mais poderosos? Deveriam cortar a língua por tanta mentira.
    Thiago: É sério, a garota ao meu lado é a filha da profecia. Nós precisamos da sua ajuda para encontrar a ${naturezas.arma}.
    ...
    Tharcísio: Filha? Vá dormir, isso não é conversa para você.
    Maria Clara: Velho desgraçado, eu sei que você fez um pacto com ELA e ofereceu a espada em troca de poder e que ela está vindo buscar agora.
    Thiago: Isso é verdade, Rei? Não acredito...
    Cibely: Nos entregue a espada!`);
    
    // Bloco 3.4
    mecanismoFala("A Batalha Final", `Tharcísio: Bom... Vocês sabem demais. Eu dou um jeito na minha filha depois. Guardas, matem-nos. 
...
function mecanismoFala(titulo, fala) {
    ...
    font-size: 24px; /* Um pouco maior para dar destaque */
    font-style: italic; 
    padding: 10px; 
...
function batalha(protagonista, inimigo) {
    ...
    while(true) {
        ...
        console.log(`%c${protagonista.nome}:\n%c❤️ ${protagonista.vida} HP   %c⚔️ Força: ${protagonista.forca}x`, estiloTitulo, estiloVida, estiloForca)
        ...
        switch (tipoAcao) {
            case "attk":
                ...
                logsdarodada += `\n\n\nVocê usou ${protagonista.habilidades[indexAcao - 1].nome} causando ${(protagonista.habilidades[indexAcao - 1].dano * protagonista.forca)} DP\n`
                break;
            case "item":
        ...
        logsdarodada += `${inimigo.nome} usou ${ataqueInimigo.nome} causando ${ataqueInimigo.dano * inimigo.forca} DP`
        if(protagonista.vida <= 0 && inimigo.vida <= 0) {
            protagonista.vida = 1
        }
        console.clear()
        console.log(logsdarodada);
...
function usarItem(nomeItem) {
    ...
    // Considerando que todos os itens são de cura
    personagens.Protagonista.vida += item.atributo;
    logItem = `\n\n\nVocê usou ${nomeItem}: restaurou ${item.atributo} de vida.`;
...
function mensagemVitoria(nomeInimigo) {
    switch (nomeInimigo) {
        case 'Boneco de madeira':
            console.log("Você derrotou o boneco de madeira! Que fácil! Se tivesse sido uma luta de verdade, teria sido uma piada... ah, espera, foi!");
            personagens.Protagonista.arma = armas.EspadaLonga;
            break;

        case 'Pedro Rocha':
            mecanismoFala("Pedro Rocha", 
                "Discurso\n" +
                "Pedro Rocha: Sempre acreditei que poderia ser um homem de bem, mas o poder e o dinheiro me corromperam. Traí meus amigos e minha ordem por ambição, mas ao final, vejo que fui apenas um tolo. " +
                "Se eu pudesse voltar atrás, eu faria tudo diferente. Apenas um verdadeiro chevalier possui a força para resistir à tentação! " +
                "Se você deseja vencer Luiza, prepare-se, pois a verdadeira batalha ainda está por vir. " +
                "E lembre-se: não acreditem no R... O verdadeiro poder está nas mãos daqueles que não se vendem!");
            console.log("Você derrotou Pedro Rocha! Ele te deixou sua espada templária, simbolizando sua luta interna e o peso de suas escolhas. Que a sabedoria de sua história te guie na busca pela verdade!");
            personagens.Protagonista.arma = armas.EspadaTemplaria;
            break;

        case 'Rei Tharcísio':
            mecanismoFala("Deus não salve o Rei!", 
                "Discurso\n" +
                "Tharcisio: Você acha que pode me vencer? Eu sou o Rei Tharcisio, o soberano absoluto do reino de Taioberen! " +
                "Sua insignificância não pode sequer compreender o poder que eu possuo! Você acha que pode se erguer contra mim? " +
                "Vocês não sabem nem mesmo o que está acontecendo em suas vidas! A verdade é que eu fui o verdadeiro governante, e você, uma mera marionete nas mãos de forças que não consegue entender! " +
                "Quando a poeira assentar, você verá que nada muda! E a história nunca esquecerá o quão grandioso eu fui. Você não é nada! NADA!");
            console.log("Você derrotou o Rei Tharcisio, um tirano que traiu seu próprio reino por poder! Agora, a Grande Excalibur Cibernética está em suas mãos, e o reino de Taioberen pode reerguer-se sob a liderança de sua filha, Maria Clara, que trará justiça e esperança ao povo!");
            personagens.Protagonista.arma = armas.GrandeExcaliburCibernetica;
            break;

        case 'luiza':
            console.log("Você derrotou Luiza, a vilã que trouxe trevas e destruição a tantos reinos. Sua crueldade não conhece limites, e ela se regozijava em causar dor e sofrimento. " +
            ...
            "Não só trouxe paz, mas também a promessa de uma nova vida para todos. Que sua coragem e determinação sejam a chama que iluminará o caminho para um novo amanhecer!");
            break;

        default:
            console.log("Inimigo desconhecido. Vitória não registrada.");
            break;
    }
}
